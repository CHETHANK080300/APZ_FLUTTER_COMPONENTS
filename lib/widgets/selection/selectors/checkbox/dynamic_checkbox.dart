...
class _DynamicCheckboxState extends State<DynamicCheckbox>
  ...
  Future<void> _loadConfigs() async {
    try {
      final String configString = await DefaultAssetBundle.of(context)
          .loadString(widget.configPath ?? 'assets/checkbox_ui_config.json');
  ...
  Widget build(BuildContext context) {
    ...
    final checkboxConfig = _config['checkbox'];
    final sizes = checkboxConfig['sizes'][widget.size];
    final colors = checkboxConfig['colors'][_getCurrentState()];
    final textConfig = checkboxConfig['text'];
            ...
            child: Row(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      ...
                      width: sizes['borderWidth'] is num
                          ? sizes['borderWidth'].toDouble()
                          : 1.0,
                    ),
                    borderRadius: BorderRadius.circular(
                        _resolveDouble(sizes['borderRadius'])),
                  ),
                  child: widget.isChecked
                      ? Icon(
                          Icons.check,
                          size: _resolveDouble(sizes['iconSize']),
                          color: _resolveColor(colors['iconColor']),
                        )
                      : null,
                ),
                // Text content
                if (widget.title != null || widget.subtitle != null) ...[
                  SizedBox(
                      width: _resolveDouble(
                          textConfig['spacing'][widget.size])),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (widget.title != null)
                          Text(
                            widget.title!,
                            style: TextStyle(
                              fontFamily: _resolveString(
                                  textConfig['fontFamily']),
                              fontSize: _resolveDouble(
                                  textConfig['fontSizes'][widget.size]),
                              color: _resolveColor(
                                widget.isDisabled
